...
Amazon Elastic Block Store (EBS) is AWS’s block storage service that provides persistent storage volumes for [EC2 instances](./elastic-compute-cloud).
...
- An EBS volume is bound to a specific [Availability Zone (AZ)](./availability-zones).
...
Because EBS volumes are tied to a single AZ, you cannot directly move a volume across AZs or [Regions](./regions).
